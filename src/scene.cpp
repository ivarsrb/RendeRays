...
void Scene::LoadFromJson(const std::string& file_name) {
    ...
    // Name
    name_ = scene_data.at("name").get<std::string>();
    ...
    // Raster size
    const std::vector<t::U16> size = scene_data.at("output_size");
    ...
    // Background color
    const std::vector<t::F32> background = scene_data.at("background");
    ...
    // Cameras
    const auto& cameras = scene_data.at("cameras");
    ...
    // Light
    const auto& light = scene_data.at("light");
    if (light.at("type").get<std::string>() == "ambient") {
        const std::vector<t::F32> color = light.at("ambient");
    ...
    else if (light.at("type").get<std::string>() == "directional") {
        const std::vector<t::F32> direction = light.at("direction");
        const std::vector<t::F32> color_ambient = light.at("ambient");
        const std::vector<t::F32> color_diffuse = light.at("diffuse");
    ...
    // Renderables
    const auto& renderables = scene_data.at("renderables");
